    ...
    private static void applyPlagueToNearbyTargets(LivingEntity target, PlayerEntity caster, StatusEffect plagueEffect, PlagueProperty plague) {
        ...
        for (LivingEntity ent : nearbyEntities) {
            ...
            if (currentEffect != null) {
                ...
                ent.addStatusEffect(new StatusEffectInstance(EffectUtils.create(plagueEffect), plague.duration_ticks, newStacks - 1));
            } else {
                ent.addStatusEffect(new StatusEffectInstance(EffectUtils.create(plagueEffect), plague.duration_ticks, 0));
            }
        }
    }
}
